    ...
    def scrape_site(self, page, url, scrape_uuid):
        ...
        try:
            ...
            for i in range(sections.count()):
                ...
                # The section header usually contains the location name
                # location_el = section.locator('xpath=.//h3[contains(@class,"opening-title")]')
                ...
                for j in range(job_items.count()):
                    ...
                    if title == "NA" or link == "NA": continue
...
class SmartRecruitersEnrichment:
    ...
    async def _extract_details(self, page):
        ...
        try:
            # Wait specifically for the location element that SmartRecruiters uses
        ...
        try:
            ...
            # SmartRecruiters splits description into specific ID sections
            for sec_id, title in [
                ...
                if await el.count() > 0:
                    ...
                    if text: desc_parts.append(f"{title}\n{text}")
            
            if desc_parts:
                data["original_description"] = "\n\n".join(desc_parts)
            else:
                # Fallback: Generic itemprop
