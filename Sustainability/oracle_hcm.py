...
def extract_site_info(jobs_page_url: str):
    """
    Extract the Oracle HCM base API URL and site_name from a jobs-page URL.

    Supported URL patterns:
      1. .../sites/<SiteName>/jobs   (named site + /jobs suffix)
      2. .../sites/<SiteName>        (named site, no /jobs suffix)
      3. .../sites/CX_N              (site number directly in path)

    The segment immediately after 'sites/' is used as site_name and as the
    initial siteNumber guess for the API call. The real siteNumber is then
    read back from the first API response.
    ...
    try:
        ...
        if idx + 1 >= len(parts):
            return None, None
        site_name = parts[idx + 1]   # e.g. "AmnealIndiaCareers" or "CX_1"
...
def seed_session(api_base: str, site_name: str) -> bool:
    """
    Use a headless Chromium browser to load the Oracle HCM careers page,
    click through any Splash Page, harvest cookies + headers, and return success.
    This bypasses Akamai and sets up a warm requests.Session.
    """
    ...
    try:
        with sync_playwright() as pw:
            ...
            # Navigate to the base UI. Do not append /jobs, as splash sites may 404
            ...
            # Splash-page handling: try clicking category links in priority order
            # to simulate a real user navigating into the full job list.
            # Priority: "All Jobs" → "New Jobs" → any link with a count like "(123)"
            _clicked = False
            try:
                for pattern in [
                    r"All\s+Jobs",
                ...
                # Last-resort: click the first anchor that has a parenthesised count e.g. "(682)"
                if not _clicked:
...
def _api_get(url: str, api_base: str, site_name: str) -> dict:
    ...
    - Attempts 1 & 2: plain requests (no cookies) — works for most Oracle HCM hosts.
...
def fetch_job_list(api_base: str, site_number: str, site_name: str, offset: int = 0) -> dict:
    """Call the Oracle HCM job listing API with retry."""
    url = (
...
def fetch_job_detail(api_base: str, job_id: str, site_number: str, site_name: str) -> dict:
    """Call the Oracle HCM job detail API with retry."""
    url = (
...
def load_target_urls_from_supabase():
    """Return list of Oracle HCM companies from companies_sustain."""
    targets = []
    try:
        response = (
...
def scrape_company(target: dict, existing_job_urls: set) -> list:
    """
    Scrape all sustainability jobs for one Oracle HCM company.
    ...
    if not api_base or not site_name:
        print(f"  -> Cannot parse Oracle HCM URL, skipping.")
        return []

    # 1. Proactively seed the session using a real browser to capture cookies 
    #    This verifies the host is alive, bypasses Akamai WAF packet dropping, 
    #    and correctly navigates Splash pages.
    is_alive = seed_session(api_base, site_name)
    
    if not is_alive and ".us2.oraclecloud" in api_base:
        print("  -> .us2. host failed or blocked Playwright. Attempting .ocs. fallback...")
        api_base = api_base.replace(".us2.", ".ocs.")
        jobs_page_url = jobs_page_url.replace(".us2.", ".ocs.")
        
        is_alive = seed_session(api_base, site_name)
        
    if not is_alive:
        print("  -> Oracle HCM endpoint is completely unresponsive. Skipping.")
        return []

    # ------------------------------------------------------------------
    # We need the siteNumber (e.g. CX_5001) which isn't in the UI URL.
    # Strategy: fetch the first page with site_name as placeholder, then
    # grab the real siteNumber from the response metadata.
    # ------------------------------------------------------------------
    site_number = site_name   # initial guess; will be corrected below
...
def run():
    ...
    if not companies:
        print("No Oracle HCM companies found in companies_sustain table.")
        return

    print(f"Found {len(companies)} Oracle HCM companies to scrape.")
    ...
    print("\nDone! All Oracle HCM jobs processed.")
